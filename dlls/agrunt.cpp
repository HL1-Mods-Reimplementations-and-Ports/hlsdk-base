...
void CAGrunt :: PrescheduleThink ( void )
{
	if ( ShouldSpeak() )
	{
		if ( m_flNextWordTime < gpGlobals->time )
		{
			...
			do
			{
				num = RANDOM_LONG(0,ARRAY_SIZE(pIdleSounds)-1);
...
void CAGrunt :: DeathSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pDieSounds[RANDOM_LONG(0,ARRAY_SIZE(pDieSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: AlertSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pAlertSounds[RANDOM_LONG(0,ARRAY_SIZE(pAlertSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: AttackSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pAttackSounds[RANDOM_LONG(0,ARRAY_SIZE(pAttackSounds)-1)], 1.0, ATTN_NORM );
}
...
void CAGrunt :: PainSound ( void )
{
	...
	EMIT_SOUND ( ENT(pev), CHAN_VOICE, pPainSounds[RANDOM_LONG(0,ARRAY_SIZE(pPainSounds)-1)], 1.0, ATTN_NORM );
}
	...
	case AGRUNT_AE_LEFT_PUNCH:
		{
			...
			if ( pHurt )
			{
				...
				EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, pAttackHitSounds[ RANDOM_LONG(0,ARRAY_SIZE(pAttackHitSounds)-1) ], 1.0, ATTN_NORM, 0, 100 + RANDOM_LONG(-5,5) );
			...
			else
			{
				...
				EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, pAttackMissSounds[ RANDOM_LONG(0,ARRAY_SIZE(pAttackMissSounds)-1) ], 1.0, ATTN_NORM, 0, 100 + RANDOM_LONG(-5,5) );
			}
		}
		break;
...
void CAGrunt :: Precache()
{
	...
	for ( i = 0; i < ARRAY_SIZE( pDieSounds ); i++ )
...
{
	{
		...
		ARRAY_SIZE ( tlAGruntSuppressHornet ),
		0,
		0,
...
Schedule_t	slAGruntRangeAttack1[] =
{
	{ 
		...
		ARRAY_SIZE ( tlAGruntRangeAttack1 ), 
		bits_COND_NEW_ENEMY			|
...
Schedule_t	slAGruntHiddenRangeAttack[] =
{
	{ 
		...
		ARRAY_SIZE ( tlAGruntHiddenRangeAttack1 ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_HEAVY_DAMAGE		|
		bits_COND_HEAR_SOUND,
...
Schedule_t	slAGruntTakeCoverFromEnemy[] =
{
	{ 
		...
		ARRAY_SIZE ( tlAGruntTakeCoverFromEnemy ), 
		bits_COND_NEW_ENEMY,
		0,
...
Schedule_t	slAGruntVictoryDance[] =
{
	{ 
		...
		ARRAY_SIZE ( tlAGruntVictoryDance ), 
		bits_COND_NEW_ENEMY		|
...
Schedule_t	slAGruntThreatDisplay[] =
{
	{ 
		...
		ARRAY_SIZE ( tlAGruntThreatDisplay ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE,
