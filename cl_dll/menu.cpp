...
int CHudMenu :: MsgFunc_ShowMenu( const char *pszName, int iSize, void *pbuf )
{
	...
	if ( m_bitsValidSlots )
	{
		...
		{  // we have the whole string, so we can localise it now
			std::strncpy( g_szMenuString, gHUD.m_TextMessage.BufferedLocaliseTextString( g_szPrelocalisedMenuString ), MAX_MENU_STRING );
			g_szMenuString[MAX_MENU_STRING - 1] = '\0';

			// Swap in characters
			if ( KB_ConvertString( g_szMenuString, &temp ) )
			{
				std::strncpy( g_szMenuString, temp, MAX_MENU_STRING );
				g_szMenuString[MAX_MENU_STRING - 1] = '\0';
				std::free( temp );
			}
		}
